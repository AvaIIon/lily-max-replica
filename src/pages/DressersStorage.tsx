...
export const DressersStorage = () => {
  ...
  // Filter products for dressers and storage category
  const dresserProducts = products.filter(product => 
    product.category.toLowerCase().includes('dresser') || 
    product.category.toLowerCase().includes('storage') ||
    product.title.toLowerCase().includes('dresser') ||
    product.title.toLowerCase().includes('drawer') ||
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <section className="bg-gradient-to-r from-purple-50 to-pink-50 py-16">
        <div className="container mx-auto px-4 text-center">
          ...
          <p className="text-xl text-muted-foreground max-w-2xl mx-auto">
            Organize your space with our stylish dressers, drawers, and storage solutions. Keep everything tidy and accessible.
          </p>
        </div>
      </section>
      ...
      <section className="container mx-auto px-4 pb-16">
        <div className="flex justify-between items-center mb-8">
          <div>
            <h2 className="text-2xl font-bold text-foreground">
              Dressers & Storage ({dresserProducts.length} products)
            </h2>
        ...
        {dresserProducts.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {dresserProducts.map((product) => {
